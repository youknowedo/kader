...
import { userTable, vendorTable } from '../../../../web/src/lib/db/schema';
...
export const queries = {
	getSingle: procedure.input(z.string()).query(
		...
		> => {
			...
			if (user.role !== 'admin' && user.role !== 'vendor' && user.vendor_id !== id)
				return {
					success: false,
					error: 'Unauthorized'
				};

			const vendor = await db.select().from(vendorTable).where(eq(vendorTable.id, id));

			const users = await db.select({}).from(userTable).where(eq(userTable.vendor_id, id));
	...
	getMultiple: procedure.input(z.array(z.string()).nullish()).query(
		async ({
			...
			for (const vendor of vendors) {
				const users = await db.select({}).from(userTable).where(eq(userTable.vendor_id, vendor.id));
				const owner = await db.select().from(userTable).where(eq(userTable.id, vendor.owner));
