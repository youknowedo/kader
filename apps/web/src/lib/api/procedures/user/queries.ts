...
export const queries = {
	getSingle: procedure
		...
		.query(
			async ({ ctx, input: id }): Promise<{ success: boolean; error?: string; user?: User }> => {
				if (!ctx.sessionId)
					return {
						success: false,
				...
				if (!id)
					return {
						...
						user: user && {
							...user,
							pfp: await minio.presignedGetObject(process.env.MINIO_BUCKET!, user.id + '.webp')
						}
					};
				...
				const selectedUser = (await db.select().from(userTable).where(eq(userTable.id, id)))[0];
	...
	getMultiple: procedure
		...
		.query(async ({ ctx, input }): Promise<ResponseData<{ users: User[] }>> => {
			if (!ctx.sessionId)
				return {
					success: false,
			...
			const pfp = await Promise.all(
				users.map((u) => minio.presignedGetObject(process.env.MINIO_BUCKET!, u.id + '.webp'))
			);
